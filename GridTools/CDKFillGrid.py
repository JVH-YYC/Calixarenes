...
def create_boltzmann_list(conformer_energy_list,
    ...
    'gentle' Boltzmann distribution: the actual difference in energy is
    divided by 2, so that rare but potentially important confromations
    are included to a degree they can be detected
    ...
    division_factor : integer
        A factor that sets the amount by which energy differences are compressed
    Returns
    -------
...
def fill_shallow_grid(grid_size,
    ...
    for atom_object in molecule.GetAtoms():
        ...
        elif atom_object.GetAtomicNum() == 17:
            prop_list.append((1.75, float(atom_object.GetProp('PartialCharge')), 14.6))
        else:
            raise ValueError('Unsupported atom type detected, atomic number:', atom_object.GetAtomicNum())
